...
export default function Leaderboard() {
  ...
  const sortedScores = BEST_SCORES.sort((a, b) => b.points - a.points);
  return (
    <div className="flex flex-col items-center justify-center gap-y-2">
      <div className="flex justify-between py-2 px-10 gap-4 text-slate-700">
        <span>Puesto</span>
        <span className="w-96">Nombre del jugador</span>
        <span className="pl-2 pr-5">Duración</span>
        <span>Puntaje</span>
      </div>
      ...
      {sortedScores.map((score, index) => {
        ...
        return (
          ...
          >
            <span className="text-3xl font-bold" title="Puesto">
              {index + 1}.
            </span>
            ...
            <div className="relative">
              ...
              {isOnline && (
                <span
                  className="size-6 bg-emerald-500 border-4 border-white rounded-full absolute -bottom-1 -right-1 hover:scale-110"
                  title="En línea"
                />
              )}
            </div>
