export default class Alert {
  ...
  showAlert (title, {
    ...
    const alertContainer = this.scene.add.container(this.scene.scale.width / 2, this.scene.scale.height / 2)
    ...
    // Título del modal
    if (title) {
      const modalTitle = this.scene.add.bitmapText(0, -height / 2 + (image ? 325 : (message ? 50 : 60)), 'primaryFont', title, 30).setOrigin(0.5, 0)
      alertContainer.add(modalTitle)
    }

    // Texto opcional
    if (message) {
      const modalText = this.scene.add.bitmapText(0, -height / 2 + (image ? 375 : 90), 'primaryFont', message, 16).setOrigin(0.5, 0)
